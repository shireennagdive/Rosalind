...
public class MiddleEdge {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(middleEdge(str[0], str[1], 5));
    }
    ...
    public static String middleEdge( String v, String w, int indel ) {
        ...
        if(max != scores.length-1) {
            if(backtrack[max] == 0) {
                next = "(" + ((max+1)-indel) + ", " + (w.length()/2+1) + ")";
            }
            else if(backtrack[max] == 1) {
                next = "(" + (max-indel) + ", " + (w.length()/2+1) + ")";
            }
            else {
                next = "(" + ((max+1)-indel) + ", " + (w.length()/2) + ")";
            }
        }
        return "(" + (max-indel) + ", " + (w.length()/2) + ") " + next;
    }
